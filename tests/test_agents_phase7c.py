...
async def test_ozymandias_buy():
    ...
    await a.logic(
        MarketData(
            price=1.0, lp_depth=60_000, meme_hype=60, whale_inflow=0, volatility=10
        )
    )
    sig = await a.logic(
        MarketData(
            price=0.99, lp_depth=60_000, meme_hype=60, whale_inflow=0, volatility=10
        )
    )
...
async def test_johan_hold_override():
    a = JohanAgent()
    md = MarketData(
        price=1, lp_depth=1_000, meme_hype=95, whale_inflow=0, volatility=10
    )
...
async def test_genghis_buy():
    a = GenghisAgent()
    md = MarketData(
        price=1, lp_depth=1_000, meme_hype=70, whale_inflow=1_500, volatility=15
    )
    sig = await a.logic(md)
    assert sig.action == "BUY"
