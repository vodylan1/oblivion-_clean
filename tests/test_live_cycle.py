...
class HoldAgent(Agent):
    meta = AgentMeta(
        name="Hold", version="0.0", risk_profile="neutral", description="no‑op"
    )

    async def logic(self, _):
        return TradeSignal(
            action="HOLD", confidence=1.0, meta={"agent": self.meta.name}
        )


@pytest.mark.asyncio
async def test_live_cycle_40_ticks():
    conductor = SynergyConductor([HoldAgent()], lambda *_: 0, decay=1.0)
