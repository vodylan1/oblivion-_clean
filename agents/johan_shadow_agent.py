...
class JohanShadowAgent(Agent):
    ...
    async def logic(self, market_data: MarketData):
        ...
        if threat_score <= 30 and market_data.meme_hype < 25:
            return TradeSignal(
                action="BUY", confidence=1.0, meta={"agent": self.meta.name}
            )
        if market_data.meme_hype > 55:
            return TradeSignal(
                action="SELL", confidence=1.0, meta={"agent": self.meta.name}
            )
        return TradeSignal(
            action="HOLD", confidence=0.3, meta={"agent": self.meta.name}
        )
